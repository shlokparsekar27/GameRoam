...
import { Send, Paperclip, Loader2, Trash2, Check, CheckCheck, MoreVertical, ArrowLeft, XCircle, Shield, Radio, FileText, Lock } from 'lucide-react';
...
const MediaMessage = ({ path, onLoaded }) => {
  ...
  if (loading) return <div className="text-[10px] text-cyber font-code animate-pulse flex items-center gap-2"><Loader2 size={10} className="animate-spin" /> DOWNLOAD_STREAM...</div>;
...
export default function Chat({ session }) {
  ...
  return (
    <div className="flex flex-col h-full bg-void-950 relative overflow-hidden">
      ...
      {/* HEADER */}
      <div className="p-3 md:p-4 border-b border-white/5 flex justify-between items-center bg-void-900/90 backdrop-blur-md z-10 sticky top-0">
        <div className="flex items-center gap-3">
          <button onClick={() => navigate('/chat')} className="md:hidden p-2 -ml-2 text-slate-400 hover:text-white transition"><ArrowLeft size={20} /></button>
          ...
          <div className="flex items-center gap-3 cursor-pointer group" onClick={() => navigate(`/user/${receiverId}`)}>
            ...
            <div className="min-w-0">
              <h3 className="font-mech font-bold text-white text-sm md:text-base truncate group-hover:text-cyber transition tracking-wide flex items-center gap-2">
              ...
              >
                ...
                <div className={`flex items-end gap-2 max-w-[85%] md:max-w-[70%]`}>
                  ...
                  <div className={`clip-chamfer p-3 md:p-4 relative border shadow-lg ${isMe
                    ...
                    {isMe && <div className="absolute bottom-0 right-0 w-2 h-2 border-b border-r border-cyber/50" />}
                    ...
                    <div className={`text-[9px] font-code flex items-center justify-end gap-1 mt-2 uppercase tracking-wider opacity-70`}>
      ...
      {/* INPUT AREA */}
      <div className="p-3 md:p-4 bg-void-900 border-t border-white/5 relative z-20">
        <form onSubmit={handleSendMessage} className="flex gap-2 items-end">
          <input type="file" ref={fileInputRef} onChange={handleFileUpload} className="hidden" accept="image/*,video/*" />

          <button type="button" onClick={() => fileInputRef.current.click()} disabled={uploading} className="p-3 text-slate-500 hover:text-cyber hover:bg-void-800 clip-chamfer transition shrink-0 border border-transparent hover:border-cyber/30 mb-[1px]">
          ...
          <div className="flex-1 relative group">
            ...
            {newMessage && <div className="absolute right-3 top-1/2 -translate-y-1/2 w-1.5 h-4 bg-cyber animate-pulse" />}
            <div className="absolute inset-0 border border-cyber/0 group-focus-within:border-cyber/30 pointer-events-none clip-chamfer transition duration-500" />
          </div>

          <button type="submit" disabled={!newMessage.trim() || uploading} className="p-3 bg-cyber hover:bg-white text-black clip-chamfer transition disabled:opacity-50 disabled:cursor-not-allowed shrink-0 font-bold shadow-neon-cyan mb-[1px]">
            <Send size={20} />
          </button>
        </form>
      </div>
