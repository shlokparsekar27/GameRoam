...
import { Heart, MessageSquare, Image as ImageIcon, Send, Loader2, Radio, TrendingUp, Shield, Flame, User, Info, X } from 'lucide-react';
...
export default function CommunityFeed({ session }) {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 pb-20">
      ...
      {/* HEADER */}
      <div className="mb-8 md:mb-10 flex justify-between items-end border-b border-white/5 pb-6">
        <div className="flex items-center gap-4">
          <div className="p-3 bg-cyber/10 border border-cyber/30 clip-chamfer">
            <Radio className="text-cyber animate-pulse" size={24} />
          </div>
          <div>
            <h1 className="text-2xl md:text-3xl font-mech font-bold text-white uppercase tracking-wider">The Arena</h1>
        ...
        <button
          ...
          className="lg:hidden flex items-center gap-2 text-cyber font-code text-[10px] font-bold uppercase border border-cyber/30 px-3 py-1.5 hover:bg-cyber/10 transition clip-chamfer"
        >
          <Info size={14} /> View Intel
        </button>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
        ...
        <div className="lg:col-span-3 space-y-8">
          ...
          <div className="bg-void-800 clip-chamfer p-1 border border-white/10 relative group">
            <div className="absolute inset-x-0 bottom-0 h-[1px] bg-gradient-to-r from-transparent via-cyber/50 to-transparent opacity-0 group-hover:opacity-100 transition duration-500" />
            <div className="bg-void-900 p-4 clip-chamfer">
              ...
              <div className="flex justify-between items-center mt-3 pl-14 pt-3 border-t border-white/5">
                ...
                <button
                  ...
                  className="bg-cyber text-black px-6 py-2 font-mech font-bold text-xs uppercase tracking-widest hover:bg-white transition flex items-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed clip-chamfer shadow-neon-cyan"
                >
              ...
              <AnimatePresence>
                {posts.map(post => (
                  <motion.div
                    ...
                    className="bg-void-800 border border-white/5 clip-chamfer overflow-hidden hover:border-cyber/30 transition duration-300 cursor-pointer group shadow-lg"
                  >
                    {/* Header */}
                    ...
                    {/* Content */}
                    <div className="p-5">
                      <p className="text-slate-300 whitespace-pre-wrap leading-relaxed text-sm font-ui">{post.content}</p>
                    </div>

                    {post.image_url && (
                      <div className="w-full bg-black border-y border-white/5 relative">
                        <div className="absolute inset-0 bg-grid-pattern opacity-10 pointer-events-none" />
                        <img src={post.image_url} className="w-full max-h-[500px] object-contain opacity-90 mx-auto" />
                      </div>
                    )}
        ...
        {showMobileIntel && (
          <div className="fixed inset-0 z-[100] lg:hidden">
            <div className="absolute inset-0 bg-void-950/80 backdrop-blur-sm" onClick={() => setShowMobileIntel(false)} />
            <div className="absolute right-0 top-0 bottom-0 w-80 bg-void-900 border-l border-white/10 p-6 overflow-y-auto animate-in slide-in-from-right duration-300 shadow-2xl">
              <div className="flex justify-between items-center mb-6">
                <h2 className="text-xl font-mech font-bold text-white uppercase tracking-wider">Sector Intel</h2>
                <button onClick={() => setShowMobileIntel(false)} className="text-slate-500 hover:text-white">
                  <X size={24} />
                </button>
              </div>
              <SidebarIntel />
