...
import { 
  ...
  Archive, DollarSign, Activity, LayoutGrid, X, ArrowDownCircle, Cpu 
} from 'lucide-react';
...
export default function Dashboard({ session }) {
  ...
  const filteredGames = games.filter(game => filter === 'All' ? true : game.listing_type === filter);
  ...
  return (
    <>
      <div className="max-w-7xl mx-auto px-4 pb-40 pt-28">
        ...
        <div className="relative mb-12 p-1 rounded-3xl bg-gradient-to-br from-white/10 to-transparent">
          <div className="relative overflow-hidden bg-void-800 rounded-[1.3rem] border border-void-900 shadow-2xl">
            ...
            <div className="relative z-10 p-6 md:p-10">
              <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-10">
                ...
                <button
                  ...
                  className="group relative px-6 py-3 bg-cyber/10 hover:bg-cyber/20 border border-cyber text-cyber font-mech font-bold tracking-widest uppercase transition-all hover:shadow-neon-cyan overflow-hidden"
                >
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                {[
                  ...
                  <div key={idx} className="bg-void-900/50 border border-white/5 p-4 relative group hover:border-white/20 transition-colors">
                    ...
                    <p className={`text-3xl font-mech font-bold ${stat.color} text-glow`}>
        ...
        <div 
          className={`sticky top-24 z-30 mb-10 transition-all duration-500 ${
            isFiltersVisible ? 'translate-y-0 opacity-100' : '-translate-y-10 opacity-0 pointer-events-none'
          }`}
        >
          <div className="bg-void-800/80 backdrop-blur-xl border border-white/10 p-1.5 rounded-xl inline-flex flex-wrap gap-1 shadow-2xl">
            {['All', 'Library', 'Rent', 'Rented In', 'Sale', 'Rented Out', 'Sold'].map((f) => (
              <button
                key={f}
                onClick={() => setFilter(f)}
                className={`px-4 py-2 rounded-lg text-xs font-mech font-bold uppercase tracking-wider transition-all border ${
                  filter === f 
        ...
        {loading ? (
          ...
          <div className="text-center py-24 border border-dashed border-void-700 rounded-3xl bg-void-900/50">
            ...
            <p className="text-slate-500 font-code text-xs mb-6">No assets found in this partition.</p>
                  ...
                  <div className="clip-chamfer bg-void-800 border border-void-700 group-hover:border-cyber/50 transition-colors duration-300 h-full">
                    ...
                    <div className="aspect-[3/4] relative overflow-hidden">
                      ...
                      <div className="absolute top-0 right-0 p-2">
                         <span className={`px-2 py-1 text-[10px] font-code font-bold border backdrop-blur-md ${
                            game.listing_type === 'Sale' ? 'bg-emerald-500/10 border-emerald-500 text-emerald-400' :
                      ...
                      {/* Hover Controls */}
                      <div className="absolute inset-0 flex items-center justify-center gap-3 opacity-0 group-hover:opacity-100 transition-opacity duration-200 bg-void-900/80 backdrop-blur-sm">
                        <button 
            ...
            <div className="w-full md:w-1/2 p-8 flex flex-col bg-void-800 relative">
              ...
              <div className="relative z-10">
                ...
                <div className="mt-8 pt-8 border-t border-dashed border-white/10">
                  <div className="flex items-center gap-4 bg-void-900/50 p-4 border border-white/5">
                    <div className="w-12 h-12 rounded bg-void-700 overflow-hidden border border-white/10">
                      {profile?.avatar_url ? (
                        <img src={profile.avatar_url} alt="Owner" className="w-full h-full object-cover" />
                      ) : (
                        <div className="w-full h-full flex items-center justify-center font-bold text-cyber bg-void-900">
                          <span className="font-mech text-xl">{profile?.username?.[0] || "U"}</span>
                        </div>
                      )}
                    </div>
                    <div>
                      <p className="font-mech font-bold text-white text-lg">OWNER ID: {profile?.username || "YOU"}</p>
                      <p className="text-xs font-code text-emerald-500 flex items-center gap-1">
