...
import { Search, MapPin, ShoppingBag, Gamepad2, Loader2, MessageCircle, X, SlidersHorizontal, Cpu, Crosshair } from 'lucide-react';
...
export default function Marketplace({ session }) {
  ...
  return (
    <div className="min-h-screen pb-20 pt-28 px-4 max-w-7xl mx-auto">
      
      {/* 1. HERO / TERMINAL HEADER */}
      <div className="relative mb-12 p-1 rounded-none clip-chamfer bg-cyber/20">
        <div className="relative overflow-hidden bg-void-900 clip-chamfer border border-cyber/30">
          <div className="absolute inset-0 bg-grid-pattern opacity-30" />
          <div className="absolute top-0 right-0 w-[300px] h-[300px] bg-cyber/10 rounded-full blur-[100px] pointer-events-none" />
          
          <div className="relative z-10 p-8 flex flex-col md:flex-row justify-between items-center gap-6">
            <div>
              <h1 className="text-4xl md:text-5xl font-mech font-bold text-white uppercase tracking-tighter mb-2">
                Global <span className="text-transparent bg-clip-text bg-gradient-to-r from-cyber to-purple-500">Exchange</span>
              </h1>
              <p className="text-slate-400 font-code text-sm flex items-center gap-2">
      ...
      <div className={`sticky top-24 z-30 mb-8 transition-all duration-500 ${isHeaderVisible ? 'translate-y-0 opacity-100' : '-translate-y-10 opacity-0 pointer-events-none'}`}>
        <div className="bg-void-800/90 backdrop-blur-xl border border-white/10 p-4 shadow-2xl shadow-black/50 clip-chamfer">
          <div className="flex flex-col md:flex-row gap-4">
            
            {/* Search Input */}
            <div className="relative flex-1 group">
              <Search className="absolute left-4 top-3.5 text-slate-500 group-focus-within:text-cyber transition" size={20} />
              <input 
                type="text" 
                placeholder="SCAN_FOR_ASSETS..." 
                value={searchText}
                onChange={e => setSearchText(e.target.value)}
                className="w-full bg-void-950/50 border border-void-700 text-white font-code pl-12 pr-4 py-3 focus:border-cyber outline-none transition placeholder:text-void-700 clip-chamfer"
              />
            </div>
            ...
            <button 
              ...
              className="md:hidden flex items-center justify-center gap-2 bg-void-700 text-white p-3 border border-white/10 clip-chamfer"
            >
            ...
            <div className={`flex-col md:flex-row gap-4 ${showFiltersMobile ? 'flex' : 'hidden md:flex'} animate-in slide-in-from-top-2 md:animate-none`}>
              <div className="relative group min-w-[140px]">
                 <MapPin className="absolute left-3 top-3.5 text-slate-500 group-focus-within:text-cyber" size={16} />
                 <input 
                  type="text" 
                  placeholder="COORDINATES..." 
                  value={locationFilter}
                  onChange={e => setLocationFilter(e.target.value)}
                  className="w-full bg-void-950/50 border border-void-700 text-sm text-white font-code pl-9 pr-3 py-3 focus:border-cyber outline-none transition clip-chamfer"
                 />
              </div>

              {[
                { val: platformFilter, set: setPlatformFilter, opts: ['All', 'PS5', 'PS4', 'Xbox', 'Switch', 'PC'] },
                { val: typeFilter, set: setTypeFilter, opts: ['All', 'Rent', 'Sale'], labels: ['ALL_TYPES', 'RENT_ONLY', 'BUY_ONLY'] },
                ...
                <select 
                  ...
                  className="bg-void-950/50 border border-void-700 text-sm text-white font-code px-4 py-3 focus:border-cyber outline-none cursor-pointer hover:bg-void-900 transition clip-chamfer appearance-none"
                >
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
          {filteredGames.map((game) => (
            ...
            >
              {/* Image */}
              <div className="h-64 relative overflow-hidden bg-void-950 border-b border-white/5">
                {game.cover_url ? (
      ...
      {selectedGame && (
        <div className="fixed inset-0 bg-void-900/90 backdrop-blur-md z-[100] flex items-center justify-center p-4 animate-in fade-in duration-300">
          <div className="bg-void-800 w-full max-w-4xl clip-chamfer border border-white/10 shadow-2xl relative flex flex-col md:flex-row max-h-[90vh] overflow-y-auto">
            ...
            <button onClick={() => setSelectedGame(null)} className="absolute top-4 right-4 z-20 p-2 bg-black/50 hover:text-flux transition text-white">
              <X size={24} />
            </button>
            ...
            {/* Data */}
            <div className="w-full md:w-1/2 p-8 bg-void-800 flex flex-col border-l border-white/5">
              <div className="mb-8">
              ...
              <div className="bg-void-900/50 p-4 border border-white/5 mb-8 clip-chamfer">
                ...
                <p className="text-slate-300 text-sm leading-relaxed">
                  Asset ready for transfer. Contact owner immediately to secure this unit.
                </p>
              </div>
